...
const TableHandler = {
  ...
  refineRowHeights(tableNode, matrix, columnWidths, minRowHeights, estimatedAvailableWidth = null) {
    ...
    for (const p of matrix.placements) {
      ...
      // For pre/pre-wrap/pre-line cells, hard breaks (<br>, \n) are part of the
      // rendered output — collapsing them via extractText would let Pretext
      // re-wrap a long flat string and overcount lines. Pull the preserved-
      // newline text and ask Pretext to respect them via whiteSpace: 'pre-wrap'.
      const cs = p.cell.styles || {};
      const isPreWrap = ['pre', 'pre-wrap', 'pre-line'].includes(cs.whiteSpace);
      const text = isPreWrap
        ? this.extractTextPreserveNewlines(p.cell)
        : this.extractText(p.cell);
      if (!text) continue;
  ...
  _buildCellWidget(cell, context, style) {
    ...
    if (canUseTHelper) {
      ...
      if (style.fontStyle === 'FontStyle.italic') tArgs.push('italic: true');

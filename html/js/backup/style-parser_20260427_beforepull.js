const StyleParser = {
  ...
  // Split by ';' respecting parens and quotes (url(data:...), "First; Second" family).
  ...
  expandBorder(value) {
    ...
    // Browser-typical mapping; CSS spec leaves exact px to UA
    ...
    // Emit shorthand AND per-side keys so both consumer paths see the value
  ...
  // Per-side keys with canonical border<Side><Attr> naming (e.g. borderTopWidth).
  ...
  expandBackground(value) {
    ...
    // Strip url(...) before color scan so filenames like "red.png" don't pollute
  ...
  // Faithful AST tokenizer of:
  //   /linear-gradient\(\s*to\s+(\w+)\s*,\s*([^,]+?)\s+(\d+(?:\.\d+)?)%\s*,\s*transparent/i
  // Same accept/reject set, same captures, same throws on non-string input.
  parseLinearGradient(value) {
    ...
    // Original regex has no anchor — try every occurrence until one matches.
    let searchFrom = 0;
    while (true) {
      ...
      // Non-greedy color: expand from length 1 until "<ws><num>%" fits after.
  ...
  // WS-split that keeps quoted strings as one token (e.g. "Times New Roman").
  ...
  expandFont(value) {
    ...
    let familyStart = -1;
    let sawSize = false;
    // Pre-size keywords (font-variant, font-stretch) — recognized so they
    // don't get mistaken for the start of font-family.
    const PRE_SIZE_KW = new Set([
      'small-caps',
      'all-small-caps', 'petite-caps', 'all-petite-caps', 'unicase', 'titling-caps',
      'ultra-condensed', 'extra-condensed', 'condensed', 'semi-condensed',
      'semi-expanded', 'expanded', 'extra-expanded', 'ultra-expanded',
    ]);
    const SIZE_KW = new Set([
    ...
    for (let i = 0; i < parts.length; i++) {
      const part = parts[i];
      const lower = part.toLowerCase();
      if (['bold','bolder','lighter','normal'].includes(lower)) {
        // Canonical lowercase so case-sensitive downstream mappers match.
        result.fontWeight = lower;
      } else if (this._isThreeDigits(part)) {
        result.fontWeight = part;
      } else if (['italic','oblique'].includes(lower)) {
        ...
        if (parsed) {
          ...
          // Looked like a size but didn't parse — treat as family start (e.g. "7Up").
          familyStart = i;
          break;
        }
      } else if (!sawSize && SIZE_KW.has(lower)) {
        // Guarded so "small"/"medium"/etc. AFTER a numeric size become family, not size.
        result.fontSize = lower;
        sawSize = true;
      } else if (!sawSize && PRE_SIZE_KW.has(lower)) {
        continue;
      } else {
        familyStart = i;
        break;
  ...
  // /^([\d.]+)(px|em|rem|pt|%)?(?:\/([\d.]+)(px|em|rem|pt|%)?)?$/  + 'normal' line-height.
  // size: defaults to 'px' if unit absent.  lineHeight: empty unit stays empty.  null on mismatch.
  _parseFontSize(s) {
    ...
    if (p < len && s[p] === '/') {
      p++;
      if (s.slice(p, p + 6).toLowerCase() === 'normal') {
        p += 6;
        lineHeight = 'normal';
      } else {
        const lhStart = p;
        while (p < len && isDigitOrDot(s[p])) p++;
        if (p === lhStart) return null;
  ...
  // Tolerates trailing whitespace + ';' (HTML attribute artifacts); rejects other junk.
  parseDimension(value) {
    ...
    const units = ['rem', 'em', 'px', 'pt', 'vh', 'vw', '%']; // longest first
    let unit = 'px';
  ...
  colorToFlutter(color) {
    ...
    if (color.startsWith('#')) {
      ...
      // Validate every char is hex; invalid input falls through to fallback.
    ...
    if (rgbaMatch) {
      // Clamp so out-of-range values don't silently wrap via Flutter's & 0xff.
      const clampCh = (n) => Math.max(0, Math.min(255, n));
      const clampAl = (n) => Math.max(0, Math.min(1, n));
      const parseChannel = (s) => clampCh(s.endsWith('%')
        ? Math.round(parseFloat(s) * 255 / 100)
        : Math.round(parseFloat(s)));
      const parseAlpha = (s) => clampAl(s.endsWith('%')
        ? parseFloat(s) / 100
        : parseFloat(s));
  ...
  // { side: 'BorderSide(...)', cssStyle: 'dashed'|'dotted'|'solid'|... }
  ...
  // { border: 'Border(...)', dash: {top,right,bottom,left} } with CSS style info per side
  ...
  // Collapsed mode: each cell draws bottom+right; top/left only on first row/col
  // (outer edge) so shared edges aren't doubled.

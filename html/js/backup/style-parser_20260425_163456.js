const StyleParser = {
  ...
  expandFont(value) {
    ...
    let familyStart = -1;
    let sawSize = false;
    // CSS pre-size keywords (font-variant, font-stretch). We don't store them,
    // but skip past them so subsequent tokens (weight, size, family) still parse.
    ...
    // CSS font-size keywords (absolute-size + relative-size). Stored as-is.
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (['bold','bolder','lighter','normal'].includes(lower)) {
        // Store canonical lowercase so downstream mappers (case-sensitive) match.
        ...
        // CSS font-size keyword (xx-small/.../xxx-large/smaller/larger) — store canonical lowercase.
        // Guarded so that "small"/"medium"/etc. appearing AFTER a numeric size are treated as family.
        result.fontSize = lower;
        sawSize = true;
      } else if (!sawSize && PRE_SIZE_KW.has(lower)) {
        // Recognized pre-size keyword (font-variant/font-stretch) — skip, keep looking.
        continue;
      } else {
  ...
  _parseFontSize(s) {
    ...
    if (p < len && s[p] === '/') {
      p++;
      // Allow CSS keyword 'normal' as line-height
      if (s.slice(p, p + 6).toLowerCase() === 'normal') {
        ...
        const lhStart = p;
        while (p < len && isDigitOrDot(s[p])) p++;
        if (p === lhStart) return null; // '/' without number/keyword
  ...
  colorToFlutter(color) {
    ...
    if (rgbaMatch) {
      ...
      // Clamp to spec range so out-of-range values don't silently wrap via Flutter's & 0xff.

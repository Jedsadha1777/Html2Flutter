                ...
                if (mergeInfo) {
                    ...
                    borderInfo.forEach(border => {
                        if (border.rangeType === 'cell' && border.value) {
                            ...
                            // Right border from last column — match luckysheet's native {style, color} format
                            if (bRow === r && bCol === lastCol && border.value.r && !cell.bd.r) {
                                cell.bd.r = {
                                    style: border.value.r.style,
                                    color: border.value.r.color
                                };
                            }
                            ...
                            if (bRow === lastRow && bCol === c && border.value.b && !cell.bd.b) {
                                cell.bd.b = {
                                    style: border.value.b.style,
                                    color: border.value.b.color
                                };
                            }
                        }
                    });
...
function getCellBorders(r, c, borderInfo, cell) {
    ...
    // Apply borderInfo in deterministic priority order so later array entries
    // can't override more-specific intent. Order:
    //   Pass 1: range-type, broad shapes  (all / outside / inside / horizontal / vertical)
    //   Pass 2: range-type, specific edges (top / bottom / left / right)
    //   Pass 3: cell-type (per-cell overrides ranges)
    // cell.bd then overrides everything (handled later).
    const BROAD_RANGE_TYPES = new Set(['border-all', 'border-outside', 'border-inside', 'border-horizontal', 'border-vertical']);
    const SPECIFIC_RANGE_TYPES = new Set(['border-top', 'border-bottom', 'border-left', 'border-right']);

    function applyRangeBorder(border) {
    ...
    // Pass 1: range-type, broad
    borderInfo.forEach(border => {
        if (border.rangeType === 'range' && BROAD_RANGE_TYPES.has(border.borderType)) {
            applyRangeBorder(border);
        }
    });

    // Pass 2: range-type, specific edges (overrides broad)
    borderInfo.forEach(border => {
        if (border.rangeType === 'range' && SPECIFIC_RANGE_TYPES.has(border.borderType)) {
            applyRangeBorder(border);
        }
    });

    // Pass 3: cell-type (overrides ranges)
    borderInfo.forEach(border => {
    ...
    if (cell.bd) {
        // Luckysheet stores per-cell borders as {style, color} (full names) — see vendor source
        // (.t/.r/.b/.l = {color: h, style: m}). Older code/data may use {s, c} shorthand —
        // support both for safety.
        const sideStyle = (s) => s?.style ?? s?.s;
        const sideColor = (s) => s?.color ?? s?.c;
        if (cell.bd.t) borders.top    = getBorderString(sideStyle(cell.bd.t), sideColor(cell.bd.t));
        if (cell.bd.b) borders.bottom = getBorderString(sideStyle(cell.bd.b), sideColor(cell.bd.b));
        if (cell.bd.l) borders.left   = getBorderString(sideStyle(cell.bd.l), sideColor(cell.bd.l));
        if (cell.bd.r) borders.right  = getBorderString(sideStyle(cell.bd.r), sideColor(cell.bd.r));

        if (cell.bd.d) {
            diagonalBorder = {
                type:  cell.bd.d.type  ?? cell.bd.d.t,
                style: sideStyle(cell.bd.d),
                color: sideColor(cell.bd.d)
            };
        }
    }
